...
def _to_number(value) -> float:
    ...
    # Убираем внутренние пробелы, чтобы опечатки вида "11. 1" читались как 11.1.
...
def clean() -> pd.DataFrame:
    ...
    # 3. Парсинг текстовых числовых колонок. Разбираем все количественные, чтобы
    # опечатки с запятой и пробелом (например "11, 1") не срывали сохранение.
    numeric_cols = [
        "pH при поступлении", "ВЕ при поступлении", "Лактат при поступлении",
        "Тяжелые гипогликемии в анамнезе (0 - нет, 1 - да)",
        "Возраст (на текущий момент)", "Возраст манифестации СД",
        "Суточная доза инсулина", "HbA1c", "Целевой HbA1c",
        "Креатинин при поступлении", "Мочевина при поступлении",
        "Калий при поступлении", "Натрий при поступлении",
    ...
    # Мочевина 86 -> 8.6 (потеря десятичной, согласовано с врачами и в отчете).
    if "Мочевина при поступлении" in df.columns:
        mask = df["Мочевина при поступлении"] == 86
        flags.append(f"Мочевина 86 -> 8.6: {int(mask.sum())}.")
        df.loc[mask, "Мочевина при поступлении"] = 8.6
    ...
    # 5. HbA1c выше 20 -> пропуск (врачи подтвердили, что это ошибка ввода).
    # Номер выносим во флаг для перепроверки первичных данных.
    if "HbA1c" in df.columns:
        high = df[df["HbA1c"] > 20]
        for _, row in high.iterrows():
            doctor_flags.append(
                f"- HbA1c {row['HbA1c']} -> пропуск: ИБ исходный {row['ИБ_исходный']}, "
                f"номер строки в датасете (N пп) {row['N пп']}")
        mask = df["HbA1c"] > 20
        flags.append(f"HbA1c выше 20 -> NaN: {int(mask.sum())}.")
